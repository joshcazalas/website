...
export const auxideMarkup = `
  <aside id="auxide-panel" class="outpost-panel auxide-panel" aria-labelledby="auxide-title" hidden>
    ...
    <div class="music-controls">
      <p class="music-instruction">Select a server to pause or skip its track.</p>
      ...
      <p id="auxide-audio-status" class="audio-note" role="status">Sound is off.</p>
    ...
    <details class="project-details"><summary>About this outpost <span>+</span></summary>
      <div class="project-detail-copy">
        <p>Auxide is my Discord music bot, written in Rust. Each production line represents a Discord server with its own queue and playback state. Pausing or skipping a track on one line doesn't affect the others.</p>
        <p class="scene-note">This demo uses Factorio's instrument samples; it doesn't connect to Discord or YouTube.</p>
      </div>
    </details>
    <a class="source-link" href="https://github.com/joshcazalas/auxide" target="_blank" rel="noopener noreferrer">See the project on GitHub <span aria-hidden="true">↗</span></a>
  </aside>
`;
...
export class AuxidePanel {
  ...
  update(snapshots: PlaybackSnapshot[], selected: number, paused: boolean, audio: AuxideAudio['state']) {
    ...
    const note = audio.status === 'error' ? 'Could not load audio. Try again.' : audio.enabled ?
      (paused ? 'Factory paused. Resume to hear the selected server.' : !current.playing ? 'This server is paused. The others keep playing.' : `Listening to Server 0${selected+1} · ${SERVERS[selected].name}`) : 'Sound is off.';
